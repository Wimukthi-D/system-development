...
router.post("/register", (req, res) => {
  ...
  bcrypt.hash(Password, 10, (err, hashedPassword) => {
    ...
    connection.query(
      "SELECT * FROM User WHERE Username = ? OR NIC = ? OR Email = ? OR PhoneNumber = ?",
      [Username, NIC, Email, PhoneNumber],
      (err, rows) => {
        if (err) {
        ...
        if (rows.length > 0) {
          ...
          connection.query(
            "INSERT INTO User (`Username`, `Password`, `FirstName`, `LastName`, `NIC`, `Email`, `PhoneNumber`, `Address`, `Usertype`,`BranchID`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)",
            [
              Username,
              hashedPassword,
              FirstName,
              ...
              Address,
              Usertype,
              Usertype === "Customer" || Usertype === "Supplier"
                ? null
                : BranchID,
            ],
            (err, result) => {
                ...
                switch (Usertype) {
                  case "Cashier":
                    connection.query(
                      ...
                      [result.insertId, registrationDate],
                      (err, result) => {
                        if (err) {
                          console.error(
                            "Error inserting into CashierTable:",
                            err
                          );
                        }
                      }
                  ...
                  case "Staff":
                    connection.query(
                      "INSERT INTO Staff (`UserId`, `hiredDate`) VALUES (?, ?)",
                      [result.insertId, registrationDate],
                      (err, result) => {
                        if (err) {
                          console.error(
                            "Error inserting into StaffTable:",
                            err
                          );
                        }
                      }
                  ...
                  case "Manager":
                    connection.query(
                      "INSERT INTO Manager (`UserId`, `hiredDate`) VALUES (?, ?)",
                      [result.insertId, registrationDate],
                      (err, result) => {
                        if (err) {
                          console.error(
                            "Error inserting into ManagerTable:",
                            err
                          );
                        }
                      }
                  ...
                  case "Supplier":
                    connection.query(
                      "INSERT INTO Supplier (`UserId`, `hiredDate`) VALUES (?, ?)",
                      [result.insertId, registrationDate],
                      (err, result) => {
                        if (err) {
                          console.error(
                            "Error inserting into SupplierTable:",
                            err
                          );
                        }
                      }
                  ...
                  case "Customer":
                    connection.query(
                      ...
                      (err, result) => {
                        if (err) {
                          console.error(
                            "Error inserting into CustomerTable:",
                            err
                          );
                        }
                      }
...
router.put("/update/:id", async (req, res, next) => {
  ...
  try {
    // Check if username, NIC, email, and phone number already exist
    connection.query(
      "SELECT * FROM User WHERE (Username = ? OR NIC = ? OR Email = ? OR PhoneNumber = ?) AND UserID != ?",
      [Username, NIC, Email, PhoneNumber, userID],
      (err, rows) => {
        if (err) {
          console.error("Error querying MySQL database:", err);
          res.status(500).send("Internal Server Error");
        ...
        if (rows.length > 0) {
          ...
          connection.query(
            "UPDATE User SET Username = ?, FirstName = ?, LastName = ?, NIC = ?, Email = ?, PhoneNumber = ?, Address = ?, Usertype = ?, BranchID = ? WHERE UserID = ?",
            [
              Username,
              FirstName,
              LastName,
            ...
            (err, result) => {
              ...
              res
                .status(200)
                .json({ message: "User updated successfully", UserId: userID });
            }
          );
        }
      }
